// Lightweight, robust YAML parser with js-yaml fallback for GCP MCP Server
const fs = require('fs');
...
function parseSimpleYaml(content) {
  ...
  for (let rawLine of lines) {
    ...
    if (trimmed.startsWith('version:')) {
      ...
      } else if (trimmed.startsWith('allowed_datasets:')) {
        currentArrayField = 'allowed_datasets';
        inProperties = false;
        inPolicies = false;
      } else if (trimmed.startsWith('allowed_tables:')) {
        currentArrayField = 'allowed_tables';
        inProperties = false;
        inPolicies = false;
